...
public class HouseMemberDocumentTest {
    ...
    private static final MockMultipartFile MULTIPART_FILE = new MockMultipartFile("memberDocument", new byte[0]);
    private static final HouseMemberDocument MEMBER_DOCUMENT = new HouseMemberDocument(MULTIPART_FILE.getName(), new byte[0]);
    ...
    void shouldGetDocumentSuccess() {
        ...
        // when
        ResponseEntity<byte[]> responseEntity = houseMemberDocumentController.getHoseMemberDocument(MEMBER_ID);
        //then
    ...
    void shouldPostDocumentSuccess() throws IOException {
        ...
        given(houseMemberDocumentService.createHouseMemberDocument(MULTIPART_FILE, MEMBER_ID))
                .willReturn(Optional.of(MEMBER_DOCUMENT));
        // when
        ResponseEntity<byte[]> responseEntity = houseMemberDocumentController.uploadHoseMemberDocument(MEMBER_ID, MULTIPART_FILE);
        //then
    ...
    void shouldPutDocumentSuccess() throws IOException {
        ...
        given(houseMemberDocumentService.updateHouseMemberDocument(MULTIPART_FILE, MEMBER_ID))
                .willReturn(Optional.of(MEMBER_DOCUMENT));
        // when
        ResponseEntity<byte[]> responseEntity = houseMemberDocumentController.updateHoseMemberDocument(MEMBER_ID, MULTIPART_FILE);
        //then
        assertEquals(HttpStatus.NO_CONTENT, responseEntity.getStatusCode());
    ...
    void shouldDeleteDocumentSuccess()  {
        ...
        given(houseMemberDocumentService.deleteHouseMemberDocument(MEMBER_ID))
                .willReturn(true);
        // when
        ResponseEntity responseEntity = houseMemberDocumentController.deleteHoseMemberDocument(MEMBER_ID);
        //then
        assertEquals(HttpStatus.NO_CONTENT, responseEntity.getStatusCode());
